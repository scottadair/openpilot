...
DISCONNECT_TIMEOUT = (
    5.0  # wait 5 seconds before going offroad after disconnect so you get an alert
)
PANDA_STATES_TIMEOUT = round(
    1000 / SERVICE_LIST["pandaStates"].frequency * 1.5
)  # 1.5x the expected pandaState frequency

ThermalBand = namedtuple("ThermalBand", ["min_temp", "max_temp"])
HardwareState = namedtuple(
    "HardwareState",
    [
        "network_type",
        "network_info",
...
def set_offroad_alert_if_changed(
    offroad_alert: str, show_alert: bool, extra_text: str | None = None
):
...
def hardware_thread(end_event, hw_queue) -> None:
    ...
    sm = messaging.SubMaster(
        ["peripheralState", "gpsLocationExternal", "selfdriveState", "pandaStates"],
        poll="pandaStates",
    )
    ...
    while not end_event.is_set():
        ...
        peripheral_panda_present = (
            peripheralState.pandaType != log.PandaState.PandaType.unknown
        )
        ...
        if sm.updated["pandaStates"] and len(pandaStates) > 0:
            ...
            in_car = (
                pandaState.harnessStatus != log.PandaState.HarnessStatus.notConnected
            )
        ...
        if (
            sm.frame % round(SERVICE_LIST["pandaStates"].frequency * DT_HW) != 0
        ) and not ign_edge:
            continue
        ...
        if fan_controller is not None:
            msg.deviceState.fanSpeedPercentDesired = fan_controller.update(
                all_comp_temp, onroad_conditions["ignition"]
            )

        is_offroad_for_5_min = (started_ts is None) and (
            (not started_seen)
            or (off_ts is None)
            or (time.monotonic() - off_ts > 60 * 5)
        )
        if is_offroad_for_5_min and offroad_comp_temp > OFFROAD_DANGER_TEMP:
            ...
            if (
                current_band.min_temp is not None
                and all_comp_temp < current_band.min_temp
            ):
                thermal_status = list(THERMAL_BANDS.keys())[band_idx - 1]
            elif (
                current_band.max_temp is not None
                and all_comp_temp > current_band.max_temp
            ):
                thermal_status = list(THERMAL_BANDS.keys())[band_idx + 1]
        ...
        startup_conditions["accepted_terms"] = (
            params.get("HasAcceptedTerms") == terms_version
        )

        # with 2% left, we killall, otherwise the phone will take a long time to boot
        startup_conditions["free_space"] = msg.deviceState.freeSpacePercent > 2
        startup_conditions["completed_training"] = (
            params.get("CompletedTrainingVersion") == training_version
        )
        ...
        startup_conditions["device_temp_engageable"] = (
            thermal_status < ThermalStatus.red
        )

        # ensure device is fully booted
        startup_conditions["device_booted"] = (
            startup_conditions.get("device_booted", False) or HARDWARE.booted()
        )
        ...
        show_alert = (
            ...
            or not startup_conditions["device_temp_engageable"]
        ) and onroad_conditions["ignition"]
        set_offroad_alert_if_changed(
            "Offroad_TemperatureTooHigh", show_alert, extra_text=extra_text
        )
                ...
                if not Path("/data/media").is_mount():
                    ...
                    try:
                        with open("/sys/block/nvme0n1/device/model") as f:
                            model = f.read().strip()
                        if (
                            not model.startswith("Samsung SSD 9")
                            and params.get("Offroad_BadNvme") is None
                        ):
        ...
        if should_start:
            ...
            if onroad_conditions["ignition"] and (
                startup_conditions != startup_conditions_prev
            ):
                cloudlog.event(
                    "Startup blocked",
                    startup_conditions=startup_conditions,
        ...
        if power_monitor.should_shutdown(
        ...
        statlog.gauge(
            "fan_speed_percent_desired", msg.deviceState.fanSpeedPercentDesired
        )
        statlog.gauge(
            "screen_brightness_percent", msg.deviceState.screenBrightnessPercent
        )
        ...
        if rising_edge_started or (count % int(600.0 / DT_HW)) == 0:
            dat = {
                ...
                "location": (
                    strip_deprecated_keys(sm["gpsLocationExternal"].to_dict())
