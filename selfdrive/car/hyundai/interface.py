...
class CarInterface(CarInterfaceBase):
    ...
    def _get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs):
        ...
        ret.radarUnavailable = (
            RADAR_START_ADDR not in fingerprint[1]
            or DBC[ret.carFingerprint]["radar"] is None
        )
        ...
        ret.openpilotLongitudinalControl = (
            experimental_long and ret.experimentalLongitudinalAvailable
        )
        ...
        if candidate in CANFD_CAR:
            ...
            if ret.flags & HyundaiFlags.CANFD_HDA2:
                ...
                if ret.flags & HyundaiFlags.CANFD_HDA2_ALT_STEERING:
                    ret.safetyConfigs[
                        -1
                    ].safetyParam |= Panda.FLAG_HYUNDAI_CANFD_HDA2_ALT_STEERING
            if ret.flags & HyundaiFlags.CANFD_ALT_BUTTONS:
                ret.safetyConfigs[
                    -1
                ].safetyParam |= Panda.FLAG_HYUNDAI_CANFD_ALT_BUTTONS
    ...
    def init(CP, logcan, sendcan):
        if CP.openpilotLongitudinalControl and not (
            ...
            disable_ecu(
                logcan, sendcan, bus=bus, addr=addr, com_cont_req=b"\x28\x83\x01"
            )
    ...
    def _update(self, c):
        ...
        allow_enable = any(
            btn in ENABLE_BUTTONS for btn in self.CS.cruise_buttons
        ) or any(self.CS.main_buttons)
        events = self.create_common_events(
            ret, pcm_enable=self.CS.CP.pcmCruise, allow_enable=allow_enable
        )
